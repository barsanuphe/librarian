...
class Search(object):
    """ This class builds a EvaluateMatch object from input conditions, then loops on all ebooks to pick out the ones who match. """
    ...
    def excludes(self, exclude_list):
        for exclude_term in exclude_list:
            ...
            if fields == []:
                self.evaluate_match.add_exclude_condition(exclude_term, None, self.is_exact)
            else:
                field, value = fields[0]
                self.evaluate_match.add_exclude_condition(value, field, self.is_exact)

    def filters(self, filter_list):
         for filter_term in filter_list:
            fields = self.field_search.findall(filter_term)
            if fields == []:
                self.evaluate_match.add_condition(filter_term, None, self.is_exact)
            else:
                field, value = fields[0]
                self.evaluate_match.add_condition(value, field, self.is_exact)

    def run_search(self, and_or): # EvaluateMatch.OR / EvaluateMatch.AND
        filtered = []
...
def match_this(ebook, value, field = None, exact = False):
    """ Try to see if an Epub object matches the condition given by value, optionnally restricted to a field. """
    value = value.lower()
    if field is None:
        # search everywhere
        ...
        for key in ebook.metadata.keys:
            ...
            if exact:
                ...
                result = result \
                        or (not is_list and any([(value == val.lower()) for val in field_value])) \
                        or (is_list     and any([(value in val.lower()) for val in field_value]))
        # tags
        if exact:
            result = result or any([(value == tag.lower()) for tag in ebook.tags])
        else:
            result = result or any([(value in tag.lower()) for tag in ebook.tags])
        ...
        if field == "tag":
            ...
            is_list = (type(field_value) == list)
            if exact:
                return (not is_list and value == field_value.lower()) or (is_list and any([(value == val.lower()) for val in field_value]))
            else:
                return (not is_list and value in field_value.lower()) or (is_list and any([(value in val.lower()) for val in field_value]))


class EvaluateMatch(object):
    """ This class builds a list of conditions, that are evaluated when is_a_match is passed with an actual Epub object. """
    OR = 1
    AND = 2

    def __init__(self):
        self.full_expression = [] # verif cond de depart, si c'est OR et tout est faux, ça ferait vrai quand meme
        self.exclude_expression = []

    def add_condition(self, value, field = None, is_exact = False):
        self.full_expression.append(lambda x: match_this(x, value, field, is_exact))

    def add_exclude_condition(self, value, field = None, is_exact = False):
        self.exclude_expression.append(lambda x: not match_this(x, value, field, is_exact))
