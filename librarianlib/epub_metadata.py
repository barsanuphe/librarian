    ...
    def parse(self):
        for node in self.metadata_element:
            ...
            if short_tag == "meta" and self.epub_version == "2.0":
                try:
                    self.metadata_dict[node.get("name").split("calibre:")[1]].append(sanitize(node.get("name").split("calibre:")[1], node.get("content"), self.author_aliases))
                except:
                   # print("non calibre: ", node.get("name"))
                    pass
            else:
                self.metadata_dict[short_tag].append(sanitize(short_tag, node.text, self.author_aliases))
        for alias in METADATA_ALIASES.keys():
            self.metadata_dict[alias] = self.metadata_dict[METADATA_ALIASES[alias]]
